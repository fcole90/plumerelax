...
# 1) run make once to create and update the po/plumerelax.pot
# 2) copy the template file and set the name to the language you want to
#    translate to:   cp po/plumerelax.pot po/en.po
# 3) edit the po file
...
po/plumerelax.pot: $(QMLJS_FILES)
	mkdir -p $(CURDIR)/po && xgettext -o po/plumerelax.pot --qt --c++ --add-comments=TRANSLATORS --keyword=tr --keyword=tr:1,2 $(QMLJS_FILES) --from-code=UTF-8
